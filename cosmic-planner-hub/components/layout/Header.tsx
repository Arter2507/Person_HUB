            ...
            {isOpen && (
                 <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-popover text-popover-foreground shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div className="p-1">
                        ...
                        <div className="h-px bg-border my-1" />
                        <a href="#" className="flex items-center w-full px-3 py-2 text-sm rounded-md text-red-500 hover:bg-accent"><LogOut className="mr-2 h-4 w-4" /><span>Sign out</span></a>
                    </div>
                 </div>
            )}
        </div>
...
const Header: React.FC<HeaderProps> = ({ onToggleMobileMenu, onToggleSidebar, isSidebarCollapsed }) => {
    return (
        <header className="h-16 flex-shrink-0 bg-card/80 backdrop-blur-sm border-b border-border flex items-center justify-between px-4 md:px-6">
